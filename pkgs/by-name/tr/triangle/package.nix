...
stdenv.mkDerivation (finalAttrs: {
  ...
  NIX_CFLAGS_COMPILE = [ ] ++ lib.optional stdenv.hostPlatform.isLinux "-DLINUX";

  buildPhase = ''
    $CC -O -std=gnu11 -o triangle triangle.c -lm
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      $CC -O -std=gnu11 -DLINUX -o showme showme.c $(pkg-config --cflags --libs x11)
    ''}
